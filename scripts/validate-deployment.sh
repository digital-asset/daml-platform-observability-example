...
until [ "$retry" -ge 5 ]; do
  ...
  prometheus_result=$(docker exec daml_observability_prometheus curl --retry 10 --retry-connrefused 'http://prometheus:9090/api/v1/query?query=daml_health_status+%3E%3D+1' || true)
  result=$($prometheus_result | grep daml_health_status)
  if [ -z "$result" ]; then
